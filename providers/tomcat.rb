...
action :before_compile do
  ...
  unless new_resource.restart_command
    new_resource.restart_command do
      run_context.resource_collection.find(:service => new_resource.tomcat_service).run_action(:restart)
    end
  end
...
action :before_deploy do
  ...
  link "#{node['tomcat']['context_dir']}/#{new_resource.application.name}.xml" do
    ...
    notifies :restart, resources(:service => new_resource.tomcat_service)
  end
end
